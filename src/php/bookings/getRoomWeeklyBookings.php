...
foreach($rooms as $roomID){
	...
	while ($tempStart <= $end){
		...
		$sth = $db->prepare("SELECT Bookings.bookingID, BookingSlots.blockID, User.uID, bookingDate, BookingSlots.roomID, startTime, endTime, reason, BookingSlots.hrsSource, User.firstName, User.lastName, COUNT(*) as numBlocks FROM Bookings JOIN BookingSlots JOIN Blocks JOIN User ON Bookings.bookingID = BookingSlots.bookingID AND BookingSlots.blockID = Blocks.blockID AND User.uID = Bookings.uID WHERE roomID = ? AND bookingDate = ? GROUP BY bookingID ASC;");
