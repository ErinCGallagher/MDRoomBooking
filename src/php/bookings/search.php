...
if ($sth->rowCount() > 0) {
	...
	while($room = $sth->fetch(PDO::FETCH_ASSOC)) {
		...
		if ($bookedBlocks < $usedBlocks) {
			...
			$sth3 = $db->prepare("SELECT Bookings.bookingID, BookingSlots.blockID, User.uID, bookingDate, BookingSlots.roomID, startTime, endTime, reason, Bookings.otherDesc, Bookings.numParticipants, BookingSlots.hrsSource, User.firstName, User.lastName, COUNT(*) as numBlocks FROM Bookings JOIN BookingSlots ON Bookings.bookingID = BookingSlots.bookingID JOIN Blocks ON BookingSlots.blockID = Blocks.blockID LEFT JOIN User ON User.uID = Bookings.uID WHERE roomID = ? AND bookingDate = ? GROUP BY bookingID ASC;");
